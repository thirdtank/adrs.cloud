...
class CustomElementTest {
  ...
  test(description,testCode) {
    it(description, () => {
      const hack = `<script>class Request {\n  constructor(url,options) { this.url = url; this.options = options; }\n}\n</script>`
      const dom = createDOM(this.html + hack,this.queryString)
      dom.window.eval(`hacks = {}`)
      dom.window.eval(`fetch = (...args) => { return hacks.fetch(...args) }`)

      dom.window.hacks.fetch = (request) => {
