      ...
      options.each do |option, option_value|
        ...
        when :required
          if option_value == true
            if value.to_s.strip == ""
              result.constraint_violation!(object: object, field: attribute, key: :required)
            end
          end
        when :minlength
          if value.respond_to?(:length) || value.nil?
            if value.nil? || value.length < option_value
              result.constraint_violation!(object: object, field: attribute, key: :too_short, context: { minlength: option_value })
            end
          else
