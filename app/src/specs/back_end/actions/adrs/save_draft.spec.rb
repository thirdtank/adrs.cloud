...
RSpec.describe Actions::Adrs::SaveDraft do
  ...
  describe "#update" do
    context "form has no external_id in it" do
      it "raises a bug" do
        account = create(:account)
        form = EditDraftAdrWithExternalIdForm.new(params: { external_id: nil})
        expect {
    ...
    context "adr does not exist" do
      it "raises not found" do
        account = create(:account)
        form = EditDraftAdrWithExternalIdForm.new(params: { external_id: "foobar"})
        expect {
          save_draft.update(form: form, account: account)
      ...
      context "account can access it" do
        context "there are constraint violations" do
          it "returns a result with the violations" do
            adr = create(:adr)
            account = adr.account
            form = EditDraftAdrWithExternalIdForm.new(params: { external_id: adr.external_id, title: "aaaaaaaaa"})
        ...
        context "there are not constraint violations" do
          it "saves, including parsing tags" do
            adr = create(:adr)
            account = adr.account
            form = EditDraftAdrWithExternalIdForm.new(params: { external_id: adr.external_id, title: "This is a test", tags: "foo, bar"})
          ...
          context "there is a refines_adr_external_id" do
            context "this account cannot access it" do
              it "sets refines_adr_id to nil" do
                ...
                adr = create(:adr)
                account = adr.account
                form = EditDraftAdrWithExternalIdForm.new(params: { external_id: adr.external_id, title: "This is a test", refines_adr_external_id: adr_being_refined.external_id})
          ...
          context "there is a replaced_adr_external_id" do
            context "this account cannot access it" do
              it "does not create a proposed_adr_replacement" do
                ...
                adr = create(:adr)
                account = adr.account
                form = EditDraftAdrWithExternalIdForm.new(params: { external_id: adr.external_id, title: "This is a test", replaced_adr_external_id: adr_being_replaced.external_id})
