...
# app and anything that needs its confdiguration, such as a CLI.
#
...
#    ```
# 2. *Configured* - Brut's static configuration is set up. See {Brut::Framework::MCP.initialize}.
#
#    ```ruby
#    configured = bootstrap.configure_only!
#    ```
# 3. *Bootstrapped* - Everything is set up and configured, services have been connected to and the app is generally ready to receive
...
class Bootstrap
  ...
  # @return [Bootstrap::ConfiguredBootstrap] that contains the {Brut::Framework::App} and {Brut::Framework::MCP}.
  def bootstrap!
    self.configure_only!
    @mcp.boot!
